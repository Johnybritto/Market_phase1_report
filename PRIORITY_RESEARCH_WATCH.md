# Priority Research Watch

Last updated: 27 August 2026

Purpose: keep a small set of high-conviction research names visible across Phase 1 and the India Bonus/Buyback/Smart Money report, especially when valuation/technical levels become attractive. This is a research-priority file, not an automatic buy list. All decisions remain subject to MASTER_INVESTMENT_RULES.md and Phase 3 validation.

## Tier A — Track Seriously on Meaningful Price Weakness

### Acutaas Chemicals
- Priority: HIGH
- Current view: strong specialty-chemicals business with semiconductor-grade photoresist exposure, but already heavily discovered/rerated.
- Action rule: do not chase momentum. Re-run Phase 3 when the stock corrects materially or earnings catch up with valuation.
- Must check: semiconductor revenue contribution, customer qualifications/repeat business, Korean JV ramp, CDMO growth, battery-chemicals contribution, EBITDA/PAT growth, cash flow, valuation versus growth, and technical support/entry zones.
- Phase 1 treatment: if price falls into a materially better valuation/support zone without thesis deterioration, surface it prominently as Deep Dive Needed / Watch for Dip rather than letting it disappear among neutral watchlist names.

### VA Tech WABAG
- Priority: HIGH
- Current view: preferred 5-year water-infrastructure business today because of stronger execution, mega-project capability, large order book, O&M annuity, cash-generation profile and desalination/reuse/high-purity-water exposure.
- Action rule: track aggressively on meaningful corrections; avoid chasing after sharp rerating.
- Must check: order intake and quality, order-book/revenue ratio, O&M share, international/sovereign-funded projects, operating cash flow, receivables, debt/net cash, margins, high-purity-water/semiconductor/solar opportunities, and technical support zones.
- Phase 1 treatment: treat meaningful corrections with intact fundamentals as a high-priority revalidation event.

### Unimech Aerospace
- Priority: HIGH
- Current view: high-quality precision-manufacturing exposure to aerospace/defence plus semiconductor-equipment supply-chain optionality, but currently discovered and expensive.
- Action rule: no chase. Re-run Phase 3 on substantial valuation/price correction or if earnings growth accelerates enough to justify the multiple.
- Must check: semiconductor-equipment revenue/customer mix, aerospace/defence mix, order book, margins, utilisation/capex, customer concentration, cash conversion, valuation and technical support.
- Phase 1 treatment: surface prominently when valuation compresses materially or price reaches a technically attractive zone without business deterioration.

## Ion Exchange — Roha Turnaround Thesis and Mandatory Revalidation Dates

### Current thesis
Ion Exchange is not currently preferred over WABAG on execution quality, but it has meaningful upside if the Roha resin facility successfully ramps. Roha is already commissioned; the investment case now depends on utilisation, fixed-cost absorption, chemicals-margin recovery, cash-flow improvement and debt stabilisation.

### What would make Ion more attractive
- Roha utilisation reaches roughly 25%+ and continues toward 40%.
- Chemicals segment margin improves sequentially.
- Consolidated EBITDA margin recovers from roughly 7.2% FY26 toward 8.5–9%+, eventually 9–10%+.
- PAT returns to sustainable double-digit growth.
- Operating cash flow becomes positive/improves.
- Gross debt stabilises and begins falling.
- Legacy low-margin engineering projects materially wind down.
- Export resin demand/customer repeat orders become visible.
- Semiconductor/high-tech UPW orders are upside, not a prerequisite for the thesis.

### Thesis failure / warning
Do not buy the turnaround merely because utilisation rises. If Roha ramps but chemicals margin, consolidated EBITDA margin and operating cash flow do not improve, the investment thesis is not validated.

### Mandatory revalidation windows
These are rough result windows and must be rechecked against the company's actual exchange-announced board/result date when available.

1. **Q2 FY27:** approximately **28 October–6 November 2026**
   - First serious checkpoint for Roha utilisation, chemicals margin, EBITDA recovery, legacy engineering drag, cash flow and debt.
2. **Q3 FY27:** approximately **25 January–2 February 2027**
   - Decisive checkpoint: confirm whether Q2 improvement is becoming a trend rather than one-quarter timing.
3. **Q4 / FY27:** approximately **25–31 May 2027**
   - Full-year validation if the thesis is still unresolved.

### Early decision rule
If by Q3 FY27 the combination is:
- Roha utilisation rising,
- chemicals margin rising,
- operating cash flow positive/improving,
then immediately re-run a full Phase 3 and compare Ion Exchange against WABAG for incremental capital.

If utilisation rises but margins/cash flow do not improve, keep WABAG preferred and do not reward the Roha narrative.

## Mandatory Water Theme Cross-Check — Applies to Every Investment Report

Water must be checked in every Phase 1, Bonus/Buyback/Smart Money, weekly holdings, weekly review and other investment-market report, even when the day's main news does not explicitly mention water.

At minimum, scan for material developments in:
- water and wastewater treatment,
- desalination,
- sewage treatment,
- industrial water and zero-liquid-discharge/reuse,
- high-purity/ultrapure water for semiconductors, electronics, solar, power, pharma and data centres,
- municipal/Jal Jeevan/AMRUT/state capex,
- pumping, membranes, resins, treatment chemicals, EPC and O&M,
- industrial capex or policy developments that create second-order water demand.

### Mandatory names
- VA Tech WABAG — always check seriously.
- Ion Exchange — always check seriously, including the Roha turnaround checkpoints.
- Scan relevant peers/beneficiaries only when evidence or sector developments justify it; examples may include EMS Ltd, Enviro Infra Engineers, Denta Water, SPML Infra/JWIL and other credible listed participants. Do not force a company into the report merely to fill the theme.

### What the water check must test
- fresh order wins/losses/cancellations and order quality,
- order book and executable revenue visibility,
- O&M/recurring-revenue share,
- revenue, EBITDA/PAT and margin direction,
- operating cash flow and cash conversion,
- receivables/working capital,
- debt/net cash,
- capacity/utilisation where relevant,
- management guidance and execution commentary,
- policy/municipal/industrial capex catalysts,
- valuation and technical entry risk,
- direct or second-order correlation from other themes such as semiconductors, data centres, power, chemicals, manufacturing, urbanisation and infrastructure.

### Reporting rule
Every investment report must include either a dedicated `Water Theme Check` section or at least one clearly labelled line stating:
- `Positive / actionable correlation`,
- `Watch / emerging correlation`,
- `Negative / thesis risk`, or
- `No material water-theme development identified`.

If the source/news does not mention water directly, perform a correlation check anyway. Clearly distinguish source-derived facts from inferred second-order correlations. Do not manufacture a connection when none exists.

## Reporting Rule — Phase 1
The daily Phase 1 workflow must explicitly check Acutaas Chemicals, VA Tech WABAG, Unimech Aerospace and Ion Exchange as priority research names even when they are not among the day's most obvious momentum names.

- Do not force them into the visible table every day.
- Surface Acutaas/WABAG/Unimech when meaningful price/valuation weakness creates a better entry setup or fundamentals materially change.
- Surface Ion around the mandatory result windows or earlier if Roha utilisation/margin/cash-flow evidence changes materially.
- For each surfaced name, use fresh price and current valuation/earnings evidence; do not rely on stale thesis notes.
- The mandatory Water Theme Cross-Check above must be completed before finalising the report.

## Reporting Rule — Bonus / Buyback / Smart Money
The daily India Bonus, Buyback & Smart Money workflow must perform a lightweight priority check on Acutaas Chemicals, VA Tech WABAG, Unimech Aerospace and Ion Exchange for:
- exchange/company filings,
- bulk/block/SAST/promoter/institutional disclosures,
- material earnings/results,
- large order wins or cancellations,
- capacity/utilisation updates,
- sector developments,
- sharp price/volume changes that need verification,
- valuation/technical correction significant enough to trigger Phase 3 revalidation.

Do not manufacture a row when there is no material event. If there is a meaningful development, flag the name clearly as a **Priority Research Watch trigger** and state whether a Phase 3 revalidation is required.

The mandatory Water Theme Cross-Check above must also be completed before finalising this report. If nothing material changed, explicitly state `No material water-theme development identified` rather than silently omitting the theme.
